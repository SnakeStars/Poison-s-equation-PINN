...
def objective(trial):
    ...
    def pdeLoss(t, lambd, build):
        ...
        masloss = torch.norm(out - f_equal, p=float('inf')).item()
    ...
    def train(lambd, build):
        ...
        for stepd in pbar:
            ...
            if stepd % 2 == 0:
                ...
                pbar.set_description("Lambda: %.4f | Step: %d | LossE: %.7f | Loss: %.7f" %
                                 (lambd, stepd, masloss, los))
    ...
    x = trial.suggest_float('lambda', 1e-1, 1e1, log=True)
    return train(x, 0)
